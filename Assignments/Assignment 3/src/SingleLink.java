...
public abstract class SingleLink<T> implements Iterable<T> {
	...
	if (this.isEmpty()) {
	    // set local front and rear to same as source
	    this.front = source.front;
	    this.rear = source.rear;
	    this.length = source.length;

	    // set source front and rear to nothing
	    source.front = null;
	    source.rear = null;
	    source.length = 0;
	}
	...
	SingleNode<T> new_front = source.front.getNext();

	source.front.setNext(this.front); // set next value of source.front to this.front
	this.front = source.front;
	this.length += 1;

	source.front = new_front;
	source.length -= 1;
