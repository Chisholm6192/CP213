...
public class SinglePriorityQueue<T extends Comparable<T>> extends SingleLink<T> {
	...
	// insert values from left first, than do right
	...
	if (this.isEmpty()) {
	...
	else { // inserting to non-empty queue
	    SingleNode<T> current = this.front;
	    SingleNode<T> previous = this.front;
	    int i = 0;

	    // search for insertion location
	    while ((data.compareTo(current.getItem()) > 0) && i < this.length) {
		previous = current;
	    ...
	    if (i == 0) {
		// if value is greatest in queue, insert to front
		node.setNext(this.front);
		this.front = node;
	    }

	    else if (i < this.length) {
		// value requires specific insertion location
		previous.setNext(node);
		node.setNext(current);
	    }

	    else if (i == this.length) {
		// if value is smallest in queue, insert at end
	...
	if (!this.isEmpty()) {
	    value = this.front.getItem(); // get value of front
	    this.front = this.front.getNext(); // set new front
	...
	while (this.length > 0) { // run until local queue has been emptied
